...
export function PredictionForm({ match }: { match: MatchSummaryViewModel | MatchDetailViewModel }) {
  ...
  return <form className="mt-5 border-t border-[var(--line)] pt-4 sm:mt-7 sm:pt-6" onSubmit={handleSubmit}>
...
function ScoreControl({ team, score, selected, onSelect, onChange }: { team: MatchSummaryViewModel["homeTeam"]; score: number; selected: boolean; onSelect: () => void; onChange: (score: number) => void }) {
  return <div><button aria-label={`Predict ${team.name} to win`} aria-pressed={selected} className={`mx-auto flex min-h-14 w-full flex-col items-center justify-center gap-1 rounded-xl border px-3 py-2 transition ${selected ? "border-[var(--lime)] bg-[var(--lime)]/10 text-[var(--lime)]" : "border-[var(--line)] bg-[var(--surface-2)] text-[var(--text-muted)] hover:border-[var(--lime)]"}`} onClick={onSelect} type="button"><CountryFlag countryCode={team.country_code} label={`${team.name} selectable flag`} /><span className="text-[10px] font-black uppercase tracking-[0.1em]">{selected ? "Winner" : "Pick winner"}</span></button><TeamCrest name={team.name} code={team.code} badgeUrl={team.badge_url} size={28} className="mx-auto mt-3" /><p className="mt-2 truncate text-center text-sm font-bold">{team.short_name}</p><div className="mt-2 flex items-center justify-center gap-1.5 sm:gap-2"><button aria-label={`Decrease ${team.short_name} score`} className="font-display h-9 w-9 rounded-xl border border-[var(--line)] bg-[var(--surface-2)] text-2xl font-bold leading-none text-[var(--text-muted)] transition hover:border-[var(--lime)] hover:text-[var(--lime)] disabled:cursor-not-allowed disabled:opacity-45 sm:h-10 sm:w-10" type="button" onClick={() => onChange(Math.max(0, score - 1))} disabled={score === 0}>−</button><input aria-label={`${team.short_name} score`} className="score-input font-display block w-14 bg-transparent text-center text-4xl sm:w-20 sm:text-6xl font-bold leading-none text-[var(--text)]" inputMode="numeric" min="0" type="number" value={score} onChange={(event) => onChange(Math.max(0, Number(event.target.value) || 0))} /><button aria-label={`Increase ${team.short_name} score`} className="font-display h-9 w-9 rounded-xl border border-[var(--line)] bg-[var(--surface-2)] text-2xl font-bold leading-none text-[var(--text-muted)] transition hover:border-[var(--lime)] hover:text-[var(--lime)] sm:h-10 sm:w-10" type="button" onClick={() => onChange(score + 1)}>+</button></div></div>;
}
