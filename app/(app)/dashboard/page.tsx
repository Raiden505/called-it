...
export default async function DashboardPage() {
  ...
  return <div className="app-content-inner space-y-8">
    ...
    <section aria-labelledby="next-call-title" className="ticket-surface ticket-rail interactive-lift overflow-hidden rounded-2xl p-5 pt-7 sm:p-7 sm:pt-9">
      ...
      <div className="mt-8 grid gap-6 md:grid-cols-[1fr_auto] md:items-end"><div><h2 id="next-call-title" className="font-display text-5xl font-bold leading-none text-[var(--text)] sm:text-6xl">{isFirstCall ? "Make the first call." : "What will you call next?"}</h2><p className="mt-4 max-w-xl text-sm leading-6 text-[var(--text-muted)]">Open the match desk to see upcoming fixtures, confidence allocations, and your saved predictions.</p></div><Button variant="primary" className="w-full sm:w-auto"><Link href="/matches">{isFirstCall ? "Make your first call" : "Open match desk"} <span aria-hidden="true">↗</span></Link></Button></div>
    ...
    <section className="grid gap-5 md:grid-cols-3"><StatCard label="Current points" value="—" detail="Scored results appear here" /><StatCard label="Global rank" value="—" detail={isFirstCall ? "Make your first call" : "Keep climbing"} /><StatCard label="Called It" value="—" detail="Exact proof is earned" /></section>
