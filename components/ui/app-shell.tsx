...
export async function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className="app-canvas"><aside className="fixed inset-y-0 left-0 z-30 hidden w-[248px] flex-col border-r border-[var(--line)] bg-[var(--surface-1)] px-6 py-7 lg:flex"><ShellBrand /><div className="mt-12"><p className="mb-3 text-[10px] font-bold uppercase tracking-[0.24em] text-[var(--text-faint)]">Matchday desk</p><nav aria-label="Primary navigation" className="space-y-1">{items.map((item) => <NavItem key={item.href} item={item} />)}</nav></div><AccountPanel displayName={displayName} username={profile?.username} avatarUrl={profile?.avatar_url} favoriteTeam={favoriteTeam} /></aside><div className="lg:hidden"><div className="flex items-center justify-between border-b border-[var(--line)] bg-[var(--surface-1)] px-4 py-4"><ShellBrand /><ProfileAvatar name={displayName} avatarUrl={profile?.avatar_url} size={36} /></div></div><main className="app-content">{children}</main><nav aria-label="Primary navigation" className="fixed inset-x-0 bottom-0 z-30 grid grid-cols-5 border-t border-[var(--line)] bg-[var(--surface-1)]/95 px-1 pb-[max(8px,env(safe-area-inset-bottom))] pt-2 backdrop-blur lg:hidden">{items.map((item) => <Link className="flex min-h-12 flex-col items-center justify-center gap-0.5 rounded-lg text-[var(--text-faint)]" href={item.href as never} key={item.href}><span aria-hidden="true" className="font-display text-lg leading-none">{item.icon}</span><span className="text-[10px] font-bold uppercase tracking-[0.08em]">{item.label}</span></Link>)}</nav></div>;
}
...
function NavItem({ item }: { item: (typeof items)[number] }) { return <Link className="group flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-semibold text-[var(--text-muted)] transition hover:bg-[var(--surface-2)] hover:text-[var(--text)]" href={item.href as never}><span aria-hidden="true" className="flex h-7 w-7 items-center justify-center rounded-md border border-transparent font-display text-lg text-[var(--text-faint)] group-hover:text-[var(--lime)]">{item.icon}</span><span>{item.label}</span></Link>; }
