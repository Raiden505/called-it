...
export function normalizeMatch(match: FootballDataMatch, configuredSeason: string): NormalizedFixture {
  ...
  const score90 = match.score.duration === "REGULAR" ? scorePair(match.score.fullTime) : scorePair(match.score.regularTime);
  const homeWon = match.score.winner === "HOME" || match.score.winner === "HOME_TEAM";
  const awayWon = match.score.winner === "AWAY" || match.score.winner === "AWAY_TEAM";
  return { provider: "football-data", externalFixtureId: String(match.id), externalCompetitionId: String(match.competition.id), season: configuredSeason, stage: match.stage ?? "REGULAR_SEASON", kickoffAt: match.utcDate ?? null, kickoffConfirmed: Boolean(match.utcDate), providerStatus: match.status, lifecycleStatus: mapProviderStatus(match.status), homeTeam: toTeam(match.homeTeam), awayTeam: toTeam(match.awayTeam), score90, scoreFinal: scorePair(match.score.fullTime), penaltyScore: scorePair(match.score.penalties), winnerExternalTeamId: homeWon ? String(match.homeTeam.id) : awayWon ? String(match.awayTeam.id) : null, events, matchPlayers: [...toMatchPlayers(match.homeTeam, String(match.homeTeam.id)), ...toMatchPlayers(match.awayTeam, String(match.awayTeam.id))], providerUpdatedAt: match.lastUpdated ?? null };
}
